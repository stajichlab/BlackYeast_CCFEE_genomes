#!/bin/bash
#SBATCH --nodes 1 --ntasks 16 --mem 96gb -p batch -J CCFEE_asm --out logs/AAFTF_filter.%a.log --time 36:00:00
...
if [[ ! -f $ASMFILE || ! -f $SORTED ]]; then    
	...
	rsync -a -v $OUTDIR/${BASE}_S*_R1_001.fastq.gz $TMPTRIM/${BASE}_R1.fq.gz
	rsync -a -v $OUTDIR/${BASE}_S*_R2_001.fastq.gz $TMPTRIM/${BASE}_R2.fq.gz
